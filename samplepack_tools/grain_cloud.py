...
class GrainCloud:
            ...
            if clip_processors is not None:
                for processor in clip_processors:
                    try:
                        ...
                        print(f'ERROR: Processor {processor} failed on clip {
                              clip.info["title"]} | {e}')

        samples = None
        if consolidate_processor is None:
            samples = np.concatenate(
                [clip.samples for clip in self.clips if clip.samples.shape[1] > 0], axis=1)
        else:
    ...
    def audio_features(self):
        ...
        for clip in self.clips:
            try:
                ...
                print(f"failed to extract features for clip {
                      clip.info['title']} | {e}")
    ...
    def plot_features_3d(self, x=('mfccs', 0), y=('mfccs', 1), z=('mfccs', 2), size='rms', color=None):
        ...
        fig = px.scatter_3d(df_fig, x='x', y='y', z='z', size=size,
                            color=color, opacity=0.8, hover_data=['title', 'duration'])
        return fig
    ...
    def compute_mfccs(self, window_size=16384, hop_size=16384, n_mfcc=10):
        ...
        for clip in self.clips:
            features = windowed_mfccs(
                clip.samples[0], window_size=window_size, hop_size=hop_size, n_mfcc=n_mfcc)
